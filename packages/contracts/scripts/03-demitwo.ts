...
const main = async () => {
  ...
  const { abi, wasm } = await getDeploymentData("demitwo");
  const demitwo = await deployContract(api, account, abi, wasm, "new", []);

  // Write contract addresses to `{contract}/{network}.ts` file(s)
  await writeContractAddresses(chain.network, { demitwo, });
};
