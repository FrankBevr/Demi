...
const main = async () => {
  ...
  const { abi, wasm } = await getDeploymentData("demione");
  const demione = await deployContract(api, account, abi, wasm, "new", []);

  // Write contract addresses to `{contract}/{network}.ts` file(s)
  await writeContractAddresses(chain.network, { demione, });
};
